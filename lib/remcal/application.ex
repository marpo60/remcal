defmodule Remcal.Application do
  # See https://hexdocs.pm/elixir/Application.html
  # for more information on OTP Applications
  @moduledoc false

  use Application

  @impl true
  def start(_type, _args) do
    children = [
      RemcalWeb.Telemetry,
      Remcal.Repo,
      {Ecto.Migrator,
        repos: Application.fetch_env!(:remcal, :ecto_repos),
        skip: skip_migrations?()},
      {DNSCluster, query: Application.get_env(:remcal, :dns_cluster_query) || :ignore},
      {Phoenix.PubSub, name: Remcal.PubSub},
      # Start the Finch HTTP client for sending emails
      {Finch, name: Remcal.Finch},
      # Start a worker by calling: Remcal.Worker.start_link(arg)
      # {Remcal.Worker, arg},
      # Start to serve requests, typically the last entry
      RemcalWeb.Endpoint
    ]

    # See https://hexdocs.pm/elixir/Supervisor.html
    # for other strategies and supported options
    opts = [strategy: :one_for_one, name: Remcal.Supervisor]
    Supervisor.start_link(children, opts)
  end

  # Tell Phoenix to update the endpoint configuration
  # whenever the application is updated.
  @impl true
  def config_change(changed, _new, removed) do
    RemcalWeb.Endpoint.config_change(changed, removed)
    :ok
  end

  defp skip_migrations?() do
    # By default, sqlite migrations are run when using a release
    System.get_env("RELEASE_NAME") != nil
  end
end
